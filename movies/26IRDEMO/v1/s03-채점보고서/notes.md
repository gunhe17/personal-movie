...
촬영 실측(2026-09-10 밤, **SPEC v6 · t10**). **테이크는 하나다** — `aireview`.

- 시작: A4 편집기, 툴바 우측 `AI 종합 리뷰`
- 종료: Ⅲ 초안이 본문에 적용된 직후
- 총 **17.20초**(t09 19.60초). 리허설·시뮬레이션 둘 다 **15단계 15.1초 · 종료 코드 0 · 오류 0**

| 구간 | 무엇 |
|---|---|
| **2.24–8.10 노컷** | 네 단계로 문서를 훑는다 → **완성도 55점 · 검사 3/3 반영 · 검토사항 3건** |
| **8.10–12.21 노컷** | 사이드바 `영역별 점수 요약`(SCT)을 집어 `4) 자기개념` 아래에 놓는다 → 표가 지면에 앉는다(5행, 화면 안에 온전히) → 사이드바에 `첨부됨` |
| **12.21–15.72 노컷** | `섹션 누락`(Ⅲ 행동 관찰) → `초안 작성` → **바뀔 문장 미리보기** → `본문에 적용` → 지적이 사라지고 **77점 · 2건**, 체크리스트에 `모든 섹션에 내용 기술`이 켜진다 |
| 15.72–16.32 | 홀드 |
...
소재는 `../_assets/구현예정.png`(1111×360 · 투명 배경).
...
| `aireview_t09` | 19.60s | 재구성 흐름 — AI 리뷰 → SCT 표 드래그 앤 드롭 → Ⅲ 초안 적용. 1176프레임(정확히 60fps) · 드롭 0 · 시드 `mindbom-2026-09-10c` · SPEC v5. DB 촬영 뒤 그대로(배터리 3종 confirmed · 로샤 22). 목업 `mockup/s03_web_aireview_t09_imac.mp4`(우측 1/3 여백). **배치가 s01과 어긋나 t10으로 대체** |
| **`aireview_t10`** | **17.20s** | **선택본 · SPEC v6 · s01과 같은 배치 규칙.** 논지 도착(분석 시작 · 완성도 점수)은 `beat` → `hold(350)`, 표 스크롤 뒤의 `beat` 제거, 안 움직이던 `reveal('초안 작성')` 제거. 충실도 **98.2%** · 드롭 0 · 노컷 **2.24–8.10 / 8.10–12.21 / 12.21–15.72** · 촬영 뒤 DB 그대로(배터리 3종 confirmed · 로샤 22) |

### 배치는 s01과 같은 규칙을 쓴다 (t10)

s01 t12의 간격에서 뽑은 규칙이다. **논지가 도착한 순간은 짧게, 상태가 바뀌는 순간에 길게.**

| 순간의 성격 | 결정 | s01 t12 | s03 t09 | **s03 t10** |
|---|---|---|---|---|
| 오프닝 도착 | `beat` | 0.71 | 0.71 | **0.73** |
| 제품이 일하는 동안 | `until` | 3.54 | 3.07 | **3.59** |
| **논지가 도착한 순간** | `until` + `hold(350)` | 0.35 | 0.71 `beat` | **0.35** |
| **상태가 바뀌는 순간** | `beat` | 0.94 | 1.40(둘이 붙음) | **1.15** |
| 화면 이동 | `scroll` 단독 | 0.84 | `reveal` 0.45+`beat` | **0.62** |
| 끝맺음 | `hold(600)` | 0.59 | 0.60 | **0.60** |

t09에서 뺀 셋:
- **분석 시작·완성도 점수 도착의 `beat`** — 도착한 화면은 아직 안 바뀐 화면이다. `hold(350)`이면 된다
- **표 스크롤 뒤의 `beat`** — `reveal`의 0.7초와 다음 `beat` 0.7초가 붙어 1.4초가 통째로 멈춰 있었다
- **`reveal('초안 작성')`** — 버튼이 이미 패널 안에 보여 스크롤이 안 일어났고(t09 실측 간격 0.02초) 0.7초 정지만 남았다

### t09 실측 (촬영본 기준)

| 구간 | 무엇 |
|---|---|
| **2.21–8.29 노컷** | `AI 종합 리뷰` → 네 단계 → **55점 · 검토사항 3건** |
| **8.29–13.43 노컷** | 드래그(10.32) → 표가 앉는다(10.96) → 스크롤·reveal(12.03) — 드롭 지점이 1쪽 끝이라 **표는 2쪽 머리**에 앉고 1쪽 꼬리와 함께 한 화면에 잡힌다 → 사이드바 `첨부됨`(13.43) |
| **13.43–17.87 노컷** | `초안 작성`(15.06) → 모달 미리보기(16.04) → `본문에 적용`(16.79) → **77점 · 2건**(17.87) |
| 17.87–18.47 | 홀드 |

Ⅲ 본문이 채워진 것 자체는 화면 밖(1쪽)이다 — 화면에 보이는 증거는 모달의 미리보기와 패널의 55→77·3→2건·체크리스트. 본문까지 보이게 하려면 적용 뒤 `reveal('.para:has-text("Ⅲ. 행동 관찰")')` 한 단계(≈1.5초)를 더해 t11을 찍는다 — **아직 안 했다**(t10은 배치만 맞췄다).

스틸: `stills/s03_web_aireview_t09_00-12-00.png`(표가 지면에 들어왔다) · `_00-16-12.png`(초안 미리보기 모달) · `_00-17-54.png`(77점 · 2건).
