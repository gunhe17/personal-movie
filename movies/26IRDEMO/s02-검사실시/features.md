...
시드가 세 아이의 로샤를 만들어 둔다 — `seed:yun-rorschach`(진행중) · `seed:jang-rorschach` · `seed:hong-rorschach`.
**s01 → s02가 데이터로 이어진다.**
...
| 시작 | `/examinations/<윤도현 로샤 examId>/collect` (마인드봄 **4503**) |
| 계정 | `_state/local-mindbom-counselor1.json` — clinician은 **본인이 examiner인 검사만** 본다 |
| 전제 | 재실행은 그 검사의 반응·영역을 지우고 — 안 그러면 반응이 쌓인다 |

리허설 실측: **24단계 28.7초 · 서버 오류 0**. DB — 자유반응·`_stt_raw`·질문·`W`·`right`·영역 전부 저장.
