...
| **`aireview_t09`** | **19.60s** | **선택본.** 재구성 흐름 — AI 리뷰 → SCT 표 드래그 앤 드롭 → Ⅲ 초안 적용. 1176프레임(정확히 60fps) · 드롭 0 · 시드 `mindbom-2026-09-10c` · SPEC v5. DB 촬영 뒤 그대로(배터리 3종 confirmed · 로샤 22). 목업 `mockup/s03_web_aireview_t09_imac.mp4`(우측 1/3 여백) |
...
Ⅲ 본문이 채워진 것 자체는 화면 밖(1쪽)이다 — 화면에 보이는 증거는 모달의 미리보기와 패널의 55→77·3→2건·체크리스트. 본문까지 보이게 하려면 적용 뒤 `reveal('.para:has-text("Ⅲ. 행동 관찰")')` 한 단계(≈1.5초)를 넣고 t10을 찍는다.
