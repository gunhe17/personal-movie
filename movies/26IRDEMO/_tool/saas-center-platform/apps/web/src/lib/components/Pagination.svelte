...
{#if totalPages > 0}
  <div class={twMerge('flex-center h-16 sm:h-20 shrink-0', $$props['class'])}>
    <div class="flex items-center space-x-1 sm:space-x-2">
      <button
        class="flex-center h-8 w-8 sm:h-10 sm:w-10 rounded-lg"
      ...
      {#each pageRange as page}
        <button
          class="flex-center h-8 w-8 sm:h-10 sm:w-10 rounded-lg text-[13px] transition duration-200 {currentPage ===
          page
