...
> **현실이지만 버그인 것은 채택하지 않는다.** 예: 헤더에 잔존하던 `text-headline-01-nomal-bold`(오타 토큰)는 다수파였지만 표준이 아니라 **수정 대상**이었다(2026-08-06 전량 `normal`로 교체·정의 삭제). "다수파 채택"은 *설계 관습*에 한하며 오타·접근성 결함에는 적용하지 않는다.
...
| **모달 제목** | `text-headline-02-normal-semibold` (20/600 고정 — §4-9. 24px은 페이지 제목 전용) |
| 카드/섹션 제목 | `text-title-01-normal-semibold` |
| 폼 라벨 | `text-body-02-normal-medium`(15) + `text-gray-700` + `mb-2` — 정본은 Web_Design §Title system 라벨 정본 |
...
| `text-headline-02-normal-semibold` | 20px | 20px | 600 | **모달 제목**(전 모달 공통, §4-9) |
...
`nomal`(오타) 형식의 레거시 alias는 **2026-08-06 전량 제거**되었다(app.css 정의 + 전 사용처).
이제 존재하지 않는 클래스이므로 쓰면 스타일이 아예 안 먹는다.

```
❌ 삭제됨 (더 이상 정의 없음)          ✅ 정식 토큰
...
같은 정리에서 **크기 위계와 어긋난 별칭**도 정정했다 — 이름이 가리키는 카테고리와 실제 px가
달라 조용한 오답을 내던 것들이다.

```
❌ 이전 (이름 ≠ 크기)                        ✅ 정정 후
text-body-03-reading-regular  = 12px   →   14px (중복 정의 제거, Body_03 스펙대로)
text-label-01-reading-medium  = 10px   →   text-caption-01-reading-medium (10px)
text-label-02-reading-medium  =  8px   →   text-caption-02-reading-medium (8px)
```

`text-label-01-reading-medium`(13px)·`text-body-03-reading-medium`(14px)은 사다리 결번이라 신규 정의했다.
...
| `primary-200` | — | **Primary CTA 비활성 면**(= `action-primary-disabled`. 라벨은 흰색 `action-primary-disabled-fg`) |
| `primary-300` | — | outline/secondary 계열의 disabled 텍스트·보더 |
...
| 카운트 행 여백 | `my-2.5` | |
| 카드 내부 구분선 여백 | `my-3` | |
| 모달 padding | `px-5 py-4`(header) · `p-5 pb-7`(body) · `px-5 pt-4 pb-5`(footer) | 좌우 20 공통 · **본문 하단만 28**, 헤더·푸터 상단만 16(§4-9) |
...
| **모달/폼 제출 버튼**(Primary CTA · 인라인 마크업) | `disabled:bg-action-primary-disabled disabled:text-action-primary-disabled-fg disabled:cursor-not-allowed`(§5-9). 🔴 2026-09-02 개정 — 옛 `bg-gray-200/text-gray-400` 표기 폐기. 활성이 `primary-500`인 버튼의 비활성은 앱 전체가 이 시맨틱 토큰 쌍(면 primary-200 · 라벨 흰색)으로 통일된다. 정본 = Web_Design.md §Components>button-primary |
| **중립 컨트롤의 비활성**(회색 버튼·칩 등) | `disabled:bg-gray-200 disabled:text-gray-400` — Primary CTA에는 쓰지 않는다 |
...
> **`white-action`(흰 버튼) 텍스트는 회색**(`gray-600`, hover `gray-800`)이다 — 파란 텍스트 아님(2026-08-04 재정의, Web_Design §button-white). 회색 배경 위 중립 액션용이라 primary CTA와 위계가 겹치면 안 된다. 동반 아이콘 색은 `var(--color-icon-primary)`로 두고 hex를 박지 않는다.
...
> **현황**: Svelte 4 문법(`export let`, `createEventDispatcher`, `on:change`) 사용 중. 이벤트는 `on:change`로 수신.
> 마이그레이션 전까지 이 패턴을 유지한다.

```svelte
<Select
  class="h-11 w-40 rounded-lg"
  options={['전체', '완료', '대기']}
...
| Prop | 타입 | 기본값 | 설명 |
|-----|------|-------|-----|
| `options` | `(string \| { value: string; title: string })[]` | `[]` | 옵션 목록 |
| `selected` | `string \| SelectOptionType \| undefined` | — | 선택된 값 |
| `placeholder` | `string` | — | 없으면 첫 번째 옵션 자동 선택 |
| `disabled` | `boolean` | `false` | 비활성 |
| `showCheck` | `boolean` | `false` | 선택 항목 체크 아이콘 |
| `showActiveHighlight` | `boolean` | `false` | 기본값과 다를 때 primary 테두리 강조 |
| `defaultValue` | `string` | — | 강조 기준 기본값 |
| `dropdownZIndex` | `number` | `10001` | 드롭다운 z-index (portal 기반) |
| `initialScrollValue` | — | — | 드롭다운 열릴 때 자동 스크롤 대상 |
| `class` | `string` | — | 컨테이너 클래스 |
| `btnClass` | `string` | — | 버튼 클래스 |

**필터 바에서 사용 시 표준**

```svelte
<!-- 필터 Select는 showActiveHighlight + defaultValue 필수 -->
<Select
  class="h-11 w-32 rounded-lg"
  options={statusOptions}
  bind:selected={filters.status}
  showActiveHighlight={true}
  defaultValue="전체"
  on:change={(e) => (filters.status = typeof e.detail === 'string' ? e.detail : e.detail.value)}
/>
```
...
>
  ...
  {#snippet footer()}
    <div class="flex justify-end gap-2">
      <button onclick={close} class="h-11 w-30 rounded-lg bg-gray-100 hover:bg-gray-200
        text-body-01-normal-medium text-gray-700">취소</button>
      <button onclick={submit} class="h-11 w-35 rounded-lg bg-primary-500 hover:bg-primary-400
...
> ⚠ **타입에만 있고 `getSizeClass`에 매핑이 없는 값**: `wideXl`·`custom500`·`tall`·`narrow`는 `size` 타입에는 선언돼 있으나 `ModalContainer.getSizeClass()`에 매핑이 없어 **조용히 `md`(max-w-md)로 떨어진다**(부록 P0). 매핑되기 전까지 위 7개만 신뢰하고 쓴다.

**⭐ 헤더 표준 (전 모달 공통)**:
- **타이틀 = `headline-02-normal-semibold`(20px / 600) 고정**. 24px(`headline-01`)은 **페이지 타이틀 전용** — 모달은 페이지 하위 컨텍스트라 한 단계 낮춘다. bold(700)·다른 크기로 갈아끼우지 않는다(§타이틀 항상 SemiBold).
  - `title` prop 사용 시 BaseModal이 자동 적용. 커스텀 `{#snippet header()}`를 쓸 때도 같은 variant를 직접 지정한다.
- **세로 정렬 = `items-center`가 기본** (BaseModal 하드코딩). 타이틀보다 닫기 아이콘(32px)이 커서 상단 정렬하면 타이틀이 위로 뜬다.
  - 예외: **타이틀 + 부제 2줄 헤더**만 `headerClass="... items-start!"`로 상단 정렬. 부제는 `body-01-normal-medium` / `body-02-regular` + `text-gray-600`.
- `headerClass`는 twMerge 병합 **맨 뒤**라 개별 오버라이드가 `!` 없이도 먹는다.

**⭐ padding 표준 — 사방 20 공통 · 본문 하단만 28**(2026-08-28 개정, 옛 본문 20 사방):
- `headerClass="px-5 py-4"` · `bodyClass="p-5 pb-7"` · `footerClass="px-5 pt-4 pb-5"`. **좌우 20은 세 영역 공통**이며 하나만 달리 주지 않는다(모달 안에서 좌우선이 어긋나 보인다).
- **헤더 상하가 16인 이유** — 닫기 아이콘(32)이 타이틀(20)보다 커서 헤더 높이를 결정한다. 상하를 키우면 헤더만 부푼다. 상하 16이면 **65**.
- **본문 하단만 28인 이유** — 상단·좌우는 헤더·푸터와 같은 20. 하단은 푸터 상단이 16으로 좁아, 한 단(8) 키워야 '본문 끝 → 액션' 경계가 선다. footer 없는 조회 전용 모달만 하단 40(`pb-10`).
- **푸터 상단만 16인 이유** — 본문 마지막 요소와 버튼 사이는 본문 하단 패딩(28)이 이미 벌려준다. 푸터 상단까지 20을 주면 하단부가 무거워진다.
- 헤더에 고정 높이(`h-22` 등)를 주지 않는다 — 높이는 패딩과 콘텐츠가 정한다. 2줄 헤더는 자연히 늘어난다.
- `title` prop만 쓰면 BaseModal이 header에 `px-5 py-4`를 자동 적용. footer 기본값은 `px-5 pt-4 pb-5`.
- `px-6`·`p-6`·`px-8`·`py-5`·`pt-10` 같은 개별 조정 금지. 이전 표준(사방 24 / `px-6 py-5`)은 폐기 — 전 모달 스윕 완료(2026-08-07).
- **섹션 구분이 필요한 body만** `bodyClass="p-0!"` + 내부 섹션이 좌우 20을 갖고 첫 섹션이 `pt-5` · 마지막 섹션이 `pb-7`을 갖는다(패딩 소유자는 항상 한 곳). 탭 바처럼 가로 전체를 쓰는 요소가 섞이면 이 형태를 쓴다.
- 커스텀 `{#snippet header()}`가 자체 패딩을 갖지 않게 한다 — 패딩은 `headerClass`가 소유(중복 시 두 배가 된다).

**footer 버튼 정렬 · 구조**(BaseModal이 소유 — 개별 모달에서 재지정하지 않는다):
- 컨테이너 = `flex items-center justify-end gap-3` + 상단 1px `border-gray-100`(`showFooterBorder={false}`로만 끈다).
- 기본: `justify-end` (취소 왼쪽, 확인 오른쪽) · 버튼 간 **gap 12**(`gap-3`)
- 삭제 버튼이 있으면: `justify-between` (삭제 왼쪽, 취소/확인 오른쪽)
- 확인·삭제 다이얼로그(`DeleteConfirmModal`·`ConfirmModal`·`CancelConfirmModal`)만 예외 — `w-full grid grid-cols-2 gap-3`로 **균등 2분할 전폭**.

**⭐ 모달 액션 버튼 규격 = 높이 44 고정**(2026-08-06 확정, 전 모달 스윕 완료):
- **`h-11`(44) + 라벨 `text-body-01-normal-medium`(16/500)** — Web_Design.md 버튼 사이즈 **Title(44)** 토큰(`44 × 20 × 10`, 레이블 Body_01 16, radius 8)이 정본.
- 40·48·52(`h-10`·`h-12`·`h-13`·`h-[52px]`)로 쓰지 않는다. 스윕 전에는 44/48/52가 뒤섞여 모달마다 하단이 다르게 무거워 보였다.
- 라벨을 `title-01-semibold`(18/600)로 키우지 않는다 — 18/600은 버튼 라벨 사다리에 없다(§4-1).
- `Button` 컴포넌트를 쓰면 기본 `size="lg"`가 h-12라 **`class="h-11"`을 명시**한다.
- 대상은 액션 버튼(확인·취소·삭제·다음/이전). 아이콘 버튼(`h-8`·`h-9`)과 선택 카드(높이 74+)는 제외.
...
> **주의**: 구형 `<svelte:fragment slot="extraBtn">` 방식은 deprecated. 신규 코드에서는 반드시 `{#snippet extraBtn()}` 방식을 사용하고, 기존 코드를 수정할 때는 마이그레이션한다.
...
- 헤더 아래 표준 여백: `className="mb-6"` (24px)
- **신규 페이지 권장 진입점.** 단 §1-2(현실 우선)에 따라 인라인 `div + h1`(`text-headline-01-normal-bold`) 헤더도 **허용**한다 — 기준 화면(assessment/status·counseling/status·member·billing)이 실제로 인라인 헤더를 쓴다. 강제 일괄 전환 대상 아님. **금지되는 건 헤더 골격 없이 `<span class="text-headline-…">`만 두는 우회뿐**(아래 §5-1 BAD)
...
BAD: <h1 class="text-headline-01-nomal-bold">  (삭제된 오타 토큰 — 스타일 미적용. normal 사용)
...
<div class="space-y-4">
  <!-- 필드 -->
  <div>
    <label for="name" class="block text-body-02-normal-medium text-gray-700 mb-2">
  ...
  <!-- textarea -->
  <div>
    <label class="block text-body-02-normal-medium text-gray-700 mb-2">메모</label>
    <textarea
...
**필드 표준**:
- label: `block text-body-02-normal-medium text-gray-700 mb-2` (15/500 — 그룹 머리글도 동일 규격. 16·14 금지)
- input 높이: `h-11`
...
```svelte
<BaseModal {modalId} size="lg" title="내담자 등록"
  showHeaderBorder={true} showFooterBorder={true}
  headerClass="px-5 py-4" bodyClass="p-5 pb-7" footerClass="px-5 pt-4 pb-5">

  {#snippet body()}
    <div class="space-y-6">
      <!-- 필드 그룹 사이 24 (5-5 패턴 사용) -->
      <!-- 딸린 체크박스 등은 <div class="space-y-3">로 함께 묶어 12 -->
    </div>
  {/snippet}
  ...
  {#snippet footer()}
    <div class="flex justify-end gap-2">
      <button onclick={close}
        class="flex h-11 w-30 items-center justify-center rounded-lg bg-gray-100
          text-body-01-normal-medium text-gray-700 hover:bg-gray-200">
        취소
      </button>
      <button onclick={submit} disabled={isSubmitting}
        class="flex h-11 w-35 items-center justify-center rounded-lg bg-primary-500
          text-body-01-normal-medium text-white hover:bg-primary-400
          disabled:bg-action-primary-disabled disabled:text-action-primary-disabled-fg disabled:cursor-not-allowed">
        {#if isSubmitting}
          <div class="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-gray-300 border-t-white"></div>
        {/if}
        저장
      </button>
    </div>
...
| 관계 | 간격 | 클래스 |
|------|------|--------|
| 서로 구분되는 필드 그룹 사이 (학교명 / 전공·학위 / 입학일 / 증빙파일) | **24** | body 래퍼에 `space-y-6` |
| 바로 위 그룹에 딸린 보조 컨트롤 (재학 중이에요 체크박스, 안내 문구) | **12** | 필드 + 컨트롤을 `<div class="space-y-3">`로 **한 덩어리로 감싼다** |
| 라벨 → 입력 | 4~8 | `mb-1` / `mb-2` |

> 🔴 **`mt-*`로 간격을 줄이려 하지 말 것.** Tailwind v4의 `space-y-*`는 v3와 반대로 **앞 형제의 `margin-bottom`**으로 간격을 만든다 — 뒤 요소에 `mt-3!`을 걸면 24가 12로 줄지 않고 **24+12=36으로 벌어진다.** 좁히려면 두 요소를 더 작은 `space-y-*` 컨테이너로 묶는다(중첩이 곧 그룹 선언이라 의미도 맞다).

`구분 간격 > 그룹 내부 간격`(최소 2배) 부등식을 유지한다 — 모두 같은 간격이면 무엇이 한 덩어리인지 사라진다. 체크박스가 위 그룹에 붙어야 그 그룹의 부속임이 읽힌다.
...
| `class="text-body-02-nomal-medium"` | `class="text-body-02-normal-medium"` | 오타 레거시(정의 삭제됨) |
...
- [ ] `nomal` 오타 토큰을 사용하지 않았는가? (정의 삭제됨 — 쓰면 스타일 미적용)
...
| 컴포넌트 | 부채 내용 | 우선순위 |
|---------|---------|---------|
| `Select.svelte` | Svelte 4 (`createEventDispatcher`, `export let`, `$$props`) | P1 |
...
| `ModalContainer.getSizeClass()` | `wideXl`, `custom500`, `tall`, `narrow` 매핑 누락 | P0 |
| ~~`nomal` 오타 타이포그래피~~ | **완료(2026-08-06)** — 전 사용처 `normal` 교체 + app.css 오타 alias 블록 삭제 | 해결 |
