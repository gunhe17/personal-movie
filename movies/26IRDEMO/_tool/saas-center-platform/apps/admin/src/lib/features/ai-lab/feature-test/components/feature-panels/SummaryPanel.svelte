<script lang="ts">
  ...
  import { SUMMARY_FLOW_STEPS } from '../../constants'
  ...
  const hasSummary = $derived(!!fnDetail?.summary)
  ...
  const flowSteps = $derived.by(() => {
    ...
    const summaryStatus = hasSummary ? 'completed'
      : runningAction === 'summary' ? 'active'
      : 'idle'
    return SUMMARY_FLOW_STEPS.map((s) => ({
      ...s,
      status: s.key === 'input' ? inputStatus : summaryStatus,
    })) as Array<{ key: string; label: string; description: string; status: 'idle' | 'active' | 'completed' | 'error' }>
  })

  const tokenItems = $derived(
    (fnDetail?.token_usage ?? []).filter((t) => t.purpose === 'field_note_summary'),
  )
  ...
  const isRunning = $derived(runningAction === 'summary')
</script>
...
<section class="section-border px-6 py-6">
  <div class="flex items-center gap-3">
    ...
    <div>
      <h3 class="text-body-01-normal-semibold text-gray-900">요약 생성</h3>
      <p class="mt-0.5 text-label-01-normal-regular text-gray-400">정제된 텍스트 → 핵심 요약 (크레딧 차감)</p>
    </div>
  </div>
  ...
  {#if fieldNoteOptions.length > 0}
    ...
    {#if fnDetail}
      <div class="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-label-01-normal-regular">
        ...
        <ModelInfoBadge model={fnDetail.summary_model ?? null} label="요약 모델" />
        {#if !hasInput}
  ...
  {:else}
    ...
    {#if selectedFieldNoteId && fieldNoteOptions.length > 0}
      <div class="mt-3">
        <button
          class="rounded-lg bg-sky-500 px-5 py-2.5 text-body-03-normal-medium text-white transition-colors hover:bg-sky-600 disabled:opacity-50"
          onclick={onRun}
          disabled={disabled || isRunning || !selectedFieldNoteId || !hasInput}
        >
          {isRunning ? '생성 중...' : '요약 생성'}
        </button>
      </div>
    {/if}
  {/if}
  ...
  {#if fnDetail?.summary}
    ...
    <div class="space-y-3">
      <ArtifactCard title="요약" type="text" content={fnDetail.summary} maxHeight="max-h-48" />
