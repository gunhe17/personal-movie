"""h07 E1 [탐색] — gemini-2.5-flash-lite 바우처 캡처 충실성.

문항 = fixtures.json 원문 슬라이스 8개(골든 = 바우처-정규화-토의.md §1~§10 실측).
판정 = 결정적 코드 채점: ① 골든 값 대조 ② quote 전건 원문 substring ③ 환각 수치 0
④ 기권 문항 null 정확. 실행: apps/api에서
    uv run python labs/assistant/hypotheses/h07-gemini25-flashlite-voucher-capture/e1/lab.py run
...
def call(
    ...
    t0 = time.time()
    r = httpx.post(E1["url"], json=body, timeout=90,
...
def run() -> None:
    ...
    with RESULTS.open("a") as f:
        for item in FIXTURES["items"]:
            for k in range(E1["K"]):
                resp, meta = call(item["text"], item["axes"], key)
                verdict = judge(item, resp)
                row = {"id": item["id"], "k": k, "meta": meta, "verdict": verdict, "response": resp}
...
def report() -> None:
    ...
    total_checks = ok_checks = q_ok = q_tot = 0
    halluc_all, cost = [], 0.0
    stable = {}
    for r in rows:
        v = r["verdict"]
        ok_checks += sum(v["checks"].values())
        total_checks += len(v["checks"])
        q_ok += v["quote_ok"]
        q_tot += v["quote_total"]
        halluc_all += [(r["id"], n) for n in v["halluc"]]
        cost += (r["meta"].get("usage") or {}).get("cost") or 0
        stable.setdefault(r["id"], []).append(tuple(sorted(v["checks"].items())))
    unstable = [i for i, ks in stable.items() if len(set(ks)) > 1]
    print(f"시행 {len(rows)} · 값정확 {ok_checks}/{total_checks} ({ok_checks/total_checks:.0%})")
    print(f"quote 원문일치 {q_ok}/{q_tot} ({q_ok/max(q_tot,1):.0%})")
    print(f"환각 수치 {len(halluc_all)}건: {halluc_all[:10]}")
    print(f"K간 판정 불안정 문항: {unstable or '없음'}")
    print(f"실비 합계 ${cost:.4f}")
    fails = {}
    for r in rows:
        for name, passed in r["verdict"]["checks"].items():
            if not passed:
                fails.setdefault(f'{r["id"]}:{name}', 0)
                fails[f'{r["id"]}:{name}'] += 1
    if fails:
        print("실패 체크:", json.dumps(fails, ensure_ascii=False, indent=1))
