...
async def notify_assessment_session_no_show_handler(
    *,
    uow: UnitOfWork,
    center_id: str,
    event_group_id: str,
    ...
    targets = await notify_single_member(
        ...
        priority="important",
        title="검사 회기 노쇼가 발생했습니다",
        body=f"검사 케이스 {case.case_code}의 회기에서 노쇼가 발생했습니다.",
        data={
            "case_id": case.id,
            "case_code": case.case_code,
            "session_id": session_id,
        },
        event_ref=f"assessment_session:{session_id}:no_show",
        atomics=atomics,
    )
    ...
    if manager_ids:
        targets.extend(
            await notify_members(
                ...
                event_type="session_no_show",
                title="검사 회기 노쇼가 발생했습니다",
                body=f"{case.case_code} 회기에서 노쇼가 발생했습니다.",
                data={
                    "case_id": case.id,
                    "case_code": case.case_code,
                    "session_id": session_id,
                },
                event_ref_prefix=f"assessment_session:{session_id}:no_show:manager",
                atomics=atomics,
            )
        )
