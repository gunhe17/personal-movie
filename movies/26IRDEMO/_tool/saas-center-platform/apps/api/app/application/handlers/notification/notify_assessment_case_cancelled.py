...
async def notify_assessment_case_cancelled_handler(
    *,
    uow: UnitOfWork,
    center_id: str,
    event_group_id: str,
    ...
    targets = await notify_single_member(
        ...
        priority="important",
        title="검사 케이스가 취소되었습니다",
        body=f"검사 케이스 {case_code}가 취소되었습니다.",
        data={"case_id": case_id, "case_code": case_code},
        event_ref=f"assessment_case:{case_id}:cancelled:{cancelled_at}",
        atomics=atomics,
    )
