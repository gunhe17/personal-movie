...
async def notify_assessment_task_submitted_handler(
    *,
    uow: UnitOfWork,
    center_id: str,
    event_group_id: str,
    ...
    targets = await notify_single_member(
        ...
        priority="normal",
        title="검사 응답이 제출되었습니다",
        body=f"검사 케이스 {case.case_code}의 검사가 제출 완료되었습니다.",
        data={"case_id": case.id, "case_code": case.case_code, "task_id": task_id},
        event_ref=f"assessment_task:{task_id}:submitted",
        atomics=atomics,
    )
