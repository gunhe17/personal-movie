...
from app.modules.billing.billable.schemas import BillableResponse
...
async def get_billable_handler(
    center_id: str,
    billable_id: str,
    uow: UnitOfWork,
) -> BillableResponse:
    facade = BillableFacade(uow)
    result = await facade.get_billable_with_response(
        center_id=center_id,
...
TOOL = {
    "name": "get_billable_handler",
    "permission": "read:billing",
    "purpose": "청구 항목(billable) 한 건의 상세를 조회한다.",
    "keywords": [
        "get billable",
        "청구 조회",
        "청구 항목 상세",
    ...
    "boundaries": "단건 청구 항목 조회(읽기 전용). 목록은 list_billables_handler.",
    "output": "청구 항목 상세 (BillableResponse).",
    "input_schema": {
        "type": "object",
        "properties": {
            "billable_id": {
