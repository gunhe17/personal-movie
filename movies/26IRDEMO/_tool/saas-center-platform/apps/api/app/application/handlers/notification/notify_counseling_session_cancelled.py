...
async def notify_counseling_session_cancelled_handler(
    *,
    uow: UnitOfWork,
    center_id: str,
    event_group_id: str,
    ...
    targets = await notify_single_member(
        ...
        body=f"상담 케이스 {case.case_code}의 회기가 취소되었습니다.",
        data={
            "case_id": case.id,
    ...
    if manager_ids:
        targets.extend(
            await notify_members(
                ...
                event_ref_prefix=f"counseling_session:{session_id}:cancelled:manager",
                atomics=atomics,
            )
        )
