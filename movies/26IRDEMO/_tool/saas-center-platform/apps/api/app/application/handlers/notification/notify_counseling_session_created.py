...
async def notify_counseling_session_created_handler(
    *,
    uow: UnitOfWork,
    center_id: str,
    event_group_id: str,
    ...
    targets = await notify_single_member(
        ...
        priority="important",
        title="새 상담 회기가 생성되었습니다",
        body=f"상담 케이스 {case.case_code}에 새 회기가 추가되었습니다.",
        data={
            "case_id": case.id,
            "case_code": case.case_code,
            "session_id": session_id,
        },
        event_ref=f"counseling_session:{session_id}:created",
        atomics=atomics,
    )
