...
async def sms_assessment_session_reminder_handler(
    *,
    uow: UnitOfWork,
    center_id: str,
    event_group_id: str,
    ...
    client_ids = (
        await case_facade.aggregate_client_ids_by_case_ids([case_id])
    ).get(case_id, [])
    ...
    targets = await resolve_client_sms_targets(
        ...
        message=build_reminder_sms_message(
            ...
            "assessment",
            schedules[0].start,
            set_summary=case.set_summary,
            assessment_summary=case.assessment_summary,
        ),
    )
