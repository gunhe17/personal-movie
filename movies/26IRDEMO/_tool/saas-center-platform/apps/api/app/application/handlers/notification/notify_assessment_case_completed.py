...
async def notify_assessment_case_completed_handler(
    *,
    uow: UnitOfWork,
    center_id: str,
    event_group_id: str,
    ...
    targets = await notify_single_member(
        ...
        priority="normal",
        title="검사 케이스가 완료되었습니다",
        body=f"검사 케이스 {case_code}의 모든 검사가 완료되었습니다.",
        data={"case_id": case_id, "case_code": case_code},
        event_ref=f"assessment_case:{case_id}:completed",
        atomics=atomics,
    )
