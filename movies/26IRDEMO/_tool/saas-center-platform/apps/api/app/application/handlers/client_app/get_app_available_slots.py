...
from app.modules.client_app.schemas import AppAvailableSlotItem, AppAvailableSlotsResponse
...
async def get_app_available_slots_handler(
    *,
    person_id: uuid_str,
    ...
    # compute
    duration_minutes = int((schedule.end - schedule.start).total_seconds() // 60)
    ...
    # return
    return AppAvailableSlotsResponse(
        date=target_date.isoformat(),
        slot_minutes=SLOT_MINUTES,
        duration_minutes=duration_minutes,
        slots=[AppAvailableSlotItem(time=slot, available=available) for slot, available in slots],
    )
