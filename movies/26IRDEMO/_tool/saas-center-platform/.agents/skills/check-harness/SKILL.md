---
name: check-harness
description: Codex 환경의 성숙도를 6축 × 3단계로 진단하고 우선순위 액션을 제시한다. 슬림 버전 — 서브에이전트 없이 단일 흐름으로 동작하며, 글로벌(User)과 프로젝트(Project) 양쪽을 점검한다.
disable-model-invocation: true
---

# check-harness — Codex 환경 성숙도 진단
...
- `/check-harness user` — 글로벌만 (`~/.Codex/`)
- `/check-harness project` — 프로젝트만 (현재 cwd의 `.Codex` + AGENTS.md)
...
**User scope:**
- `~/.Codex/AGENTS.md`, `~/.Codex/*.md` (import 포함)
- `~/.Codex/skills/`, `~/.Codex/agents/`, `~/.Codex/commands/`
- `~/.Codex/plugins/`
- `~/.Codex/settings*.json`

**Project scope:**
- `<repo>/AGENTS.md` (루트), `<repo>/apps/*/AGENTS.md`
...
- `.Codex/` 폴더 mtime으로 최근 갱신 여부
- 메모리 디렉토리에 메모가 쌓이고 있는지 (`~/.Codex/projects/<encoded>/memory/`)
...
- **WEAK_PASS** — 형식은 갖췄지만 실효성이 약함 (예: AGENTS.md는 있는데 내용이 빈약)
- **FAIL** — 증거 없음
...
| 항목 | 판정 | 증거 |
|------|------|------|
| 명확한 디렉토리 구조 | ✅ PASS | `AGENTS.md:5` 트리 다이어그램 + 실제 구조 일치 |
| ... | ... | ... |
