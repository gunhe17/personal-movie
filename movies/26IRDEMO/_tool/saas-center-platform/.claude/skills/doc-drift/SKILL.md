---
name: doc-drift
description: Claude Code가 컨텍스트로 로드하는 문서들(CLAUDE.md, 메모리, .claude/skills·agents·commands, 플러그인)을 스캔해 outdated·conflict·risky/ambiguous 세 종류의 드리프트를 찾는다. 모든 발견은 file:line 증거 필수. 솔루션 프레이밍.
disable-model-invocation: true
---

# doc-drift — Claude 컨텍스트 문서 감사

Claude가 실제로 읽어들이는 문서가 코드·설정과 어긋나거나, 서로 충돌하거나, 위험·모호한 표현을 담고 있는지 점검한다.
...
Claude가 실제로 로드하는 파일들을 빠짐없이 모은다. **읽는 건 메타 정보(파일 경로·구조)와 본문이지, 사용자 프롬프트가 아니다.**

### 글로벌 (User scope)
- `~/.claude/CLAUDE.md`
- `~/.claude/*.md` (SuperClaude 프레임워크 등 import된 것들)
- `~/.claude/skills/`, `~/.claude/agents/`, `~/.claude/commands/`
- `~/.claude/projects/<encoded-cwd>/memory/MEMORY.md` 및 같은 폴더의 메모리 파일들

### 프로젝트 (Project scope)
- `<repo>/CLAUDE.md` (루트)
- `<repo>/apps/*/CLAUDE.md` (모노레포: web/mobile/api 등 nested CLAUDE.md)
- `<repo>/.claude/skills/`, `<repo>/.claude/agents/`, `<repo>/.claude/commands/`
- `<repo>/.claude/settings*.json`

### 플러그인
- `~/.claude/plugins/` 하위에 설치된 플러그인의 SKILL.md / AGENT.md / commands
...
예시:
- CLAUDE.md가 `apps/web/src/routes/(protected)/clients/` 라고 하는데 폴더가 이미 옮겨졌거나 이름이 바뀜
...
예시:
- 루트 CLAUDE.md는 "Service 간 호출 금지", 다른 가이드는 "Service에서 다른 Service 호출 가능"
- 글로벌 RULES.md는 "TodoWrite for >3 step tasks", 프로젝트 가이드는 "5단계 이상에서만"
- 모바일 CLAUDE.md는 "border-gray-200", 다른 섹션은 "border 사용 금지"
...
1. **Scope** — 매 대화마다 로드되는 파일(`CLAUDE.md`, `MEMORY.md`, 글로벌 `RULES.md` 등)일수록 위
...
### O-1. [HIGH] <짧은 제목>
**주장 (문서)**: `CLAUDE.md:42` — "X is at /a/b/c"
**실제 (코드)**: `apps/web/src/.../x.ts:1` — 경로가 /a/b/d 로 이동
**제안**: CLAUDE.md:42를 `/a/b/d`로 갱신 → 새 입사자가 잘못된 경로를 찾지 않음
...
**문서 A**: `CLAUDE.md:120` — "Service 간 호출 금지"
...
## 적용 제안 (선택)
- 일괄 수정 PR 초안: `claude /doc-drift apply` (구현 시)
...
- **요약 문서를 존중**. CLAUDE.md가 다른 문서를 가리키기만 하는 경우는 정상. **의미상 어긋남(semantic drift)** 만 발견으로 본다.
- **고임팩트 우선**. CLAUDE.md / MEMORY.md / RULES.md의 드리프트가 가장 위험.
...
- **루트 CLAUDE.md ↔ apps/mobile/CLAUDE.md** — 같은 도메인을 양쪽에서 다르게 설명하는지 (디자인 시스템, 커밋 prefix 등)
...
- **메모리 stale 여부** — `~/.claude/projects/.../memory/`의 project 메모리가 명시한 사실(파일 경로·정책)이 아직 유효한지
