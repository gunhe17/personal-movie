...
이 프로젝트는 **SaaS web = 관리자(manager) + 상담사(counselor) + super_admin 다중 역할, mobile = 상담사(counselor) 전용**이다 (root `CLAUDE.md` / `apps/web/CLAUDE.md` / `apps/mobile/CLAUDE.md`에 명시). 비교는 항상 다음 원칙을 따른다.
...
- **mobile 코드의 `useRole` 헬퍼(isAdmin/isManager/hasFullAccess)에 헷갈리지 말 것.** mobile에 해당 헬퍼가 노출돼 있어도 **설계 의도는 counselor 전용**이다 (mobile CLAUDE.md 명시). 헬퍼는 향후 확장·임시 안전망 용도이며, 이를 근거로 "mobile도 멀티 역할 앱"이라고 결론 짓지 않는다. 헤더 문구 분기처럼 헬퍼가 실제 사용되는 경우는 §9 mobile-only로 기록하되, 본 비교의 기준은 항상 counselor.
...
- **모바일 디자인 시스템 규칙은 본 비교에서 강요하지 않는다.** "web에 있으니 mobile에도 추가" 했을 때 모바일 CLAUDE.md의 공용 컴포넌트·디자인 시스템과 충돌하는 경우 §7(검토 필요)로 분류.
