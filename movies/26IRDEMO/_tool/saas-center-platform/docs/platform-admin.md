...
| 역할 | 코드 | 권한 범위 |
|------|------|-----------|
| 일반 사용자 | `null` | 센터 기능만 사용 |
| 플랫폼 관리자 | `platform_admin` | 운영 기능 (센터 관리, 통계, 모니터링) |
| 플랫폼 슈퍼관리자 | `platform_superadmin` | 전체 기능 + 관리자 계정 관리 + 전역 설정 |
...
#### 4.3.2 관리자 계정 관리 (슈퍼관리자 전용)

```
GET    /api/v1/admin/admins
POST   /api/v1/admin/admins/invite
DELETE /api/v1/admin/admins/{account_id}/revoke
```

슈퍼관리자만 다른 관리자 계정을 초대/해제할 수 있음.
...
| 페이지 | 주요 기능 | Phase |
|--------|----------|-------|
| `/login` | 관리자 로그인 (이메일+비밀번호) | 1 |
| `/applications` | 센터 신청 목록, 상세 보기, 승인/거절 | 1 |
| `/centers` | 센터 목록, 검색, 상태 필터 | 1 |
| `/centers/[id]` | 센터 상세 (메타+멤버), 정지/활성화 | 1 |
| `/accounts` | 계정 목록, 검색, 잠금/해제/강제 로그아웃 | 1 |
| `/assessments` | 검사 도구 CRUD | 1 |
| `/stats` | 대시보드 (차트, 추이) | 2 |
| `/audit-logs` | 감사 로그 조회/검색 | 2 |
| `/messaging` | 메시징 발송 현황/에러 모니터링 | 2 |
| `/settings` | 전역 설정 관리 | 3 |
