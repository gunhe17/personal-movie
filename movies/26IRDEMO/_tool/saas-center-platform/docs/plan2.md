...
Voucher (바우처) - 간단 모델
└── Counseling과 N:N 관계 (counseling_vouchers 중간 테이블)
```
...
### Session (회기)
```
GET    /api/counselings/:id/sessions       # 상담의 회기 목록
POST   /api/counselings/:id/sessions       # 회기 추가
PUT    /api/sessions/:id                   # 회기 수정
DELETE /api/sessions/:id                   # 회기 삭제
```
...
| `CounselingForm` | 상담 등록/수정 폼 |
| `ClientSelector` | 내담자 선택 컴포넌트 |
| `CounselorChips` | 담당자 선택 (칩 형태) |
| `RoomSelector` | 상담실 선택 |
| `DurationSelector` | 상담 소요시간 선택 (30/60/90/120/180/240분) |
