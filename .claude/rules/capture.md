...
2. 뷰포트 1600×900 @2x · 60fps · light 테마 · 타이밍 상수는 러너 `SPEC`이 정본이다(현재 **v8**). 지연은 **흐름을 알아보는 최소 길이**로 잡는다 — 제품을 기다릴 때는 고정 `hold`가 아니라 `until(조건)`을 쓰고, 입력 폼은 `reveal`로 아래쪽 요소까지 보여준다. 장면 스크립트나 명령줄에서 바꾸지 않는다. 바꿔야 하면 `SPEC.version`을 올리고 이 파일과 SKILL.md를 같이 고친다.
2-2. **스크롤은 목표를 정하고 한 번에 부드럽게**(v8). 거리(px)가 아니라 도착할 요소로 `h.scrollTo(sel, note, block)`을 쓴다. 같은 목표로 `scroll`·`reveal`을 이어 붙이지 않는다. 손으로 잰 `h.scroll(dy)`는 쓰지 않는다.
...
8. 폰(시뮬레이터)은 `capture-phone.mjs`로만 찍는다 — `simctl recordVideo`·QuickTime 녹화를 쓰지 않는다. 조작은 idb 스크립트가 원칙이고 `--manual`은 idb가 없을 때의 예외다. 폰도 화면 전환은 고정 `hold`가 아니라 `p.until(라벨)`로 기다린다(규칙 2와 같다). 폰 타이밍은 러너 `SPEC_PHONE`이 정본이다(현재 **v5** — 탭 마크가 좌표 `x`·`y`(idb 포인트)와 **손가락이 닿은 시각**을, meta가 `capture.pxPerPoint`를 남긴다. 무대가 그 자리 그 시각에 탭 링을 그린다). 테이크는 **전환 구간의 고유 프레임을 세어** 쓴다(SKILL.md 6-1) — 시뮬레이터가 굶으면 `notReady 0`인데도 움직임만 끊긴다 — 규칙 2와 같이 바꿀 때는 버전을 올린다. 실기기 녹화(s06)만 iOS 화면 기록을 쓰되, 파일을 `raw/`에 넣은 뒤 반드시 `capture.mjs --manual`로 meta와 manifest를 등록한다.
