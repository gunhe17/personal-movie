...
8. 폰(시뮬레이터)은 `capture-phone.mjs`로만 찍는다 — `simctl recordVideo`·QuickTime 녹화를 쓰지 않는다. 조작은 idb 스크립트가 원칙이고 `--manual`은 idb가 없을 때의 예외다. 폰 타이밍은 러너 `SPEC_PHONE`이 정본이다(현재 **v2**) — 규칙 2와 같이 바꿀 때는 버전을 올린다. 실기기 녹화(s06)만 iOS 화면 기록을 쓰되, 파일을 `raw/`에 넣은 뒤 반드시 `capture.mjs --manual`로 meta와 manifest를 등록한다.
