...
| `taps` · `tapScale` · `tapRadius` · `tapDur` · `tapOpacity` · `tapWidth` | **phone 무대 — 탭 링.** 시뮬레이터의 터치 표시는 작고 흐린 회색 점이라 4K 무대 안에서 안 보인다 — 손끝만 한 원이 탭 자리에서 한 번 퍼지며 **처음부터 옅어진다**(흰 띠 + 잉크 테라 밝은 시트·어두운 녹음 화면 양쪽에서 보인다. 가운데 점은 없다 — 링만으로 읽힌다). **절제가 기본이다**: 눌렀다는 걸 알아챌 만큼만이고, 더 세게 해야 하면 `tapOpacity`를 컷에서 올린다. `taps`는 **촬영 meta 경로**(spec 기준 상대경로)를 주는 것이 정석이다 — `steps[]`의 누른 마크와 배율을 그대로 읽는다. 누른 마크는 시뮬레이터의 `tap`과 **폰 프로파일 웹의 `click`** 둘 다이고(폰 모양 화면끼리 표시를 통일한다), 배율은 `capture.pxPerPoint`(시뮬레이터 2) ?? `viewport.dpr`(폰 프로파일 웹 3)다 — **무대가 배율을 손으로 받지 않는다.** `tapRadius`가 논리 좌표(pt)라 두 기기의 화면 폭(402pt · 390pt)이 거의 같은 만큼 링도 화면에서 같은 크기다(실측 둘 다 무대 지름 96px). 웹 meta는 `capture-service` SPEC **v9** 이상이어야 한다 — v8 이하는 `click`에 좌표가 아예 없다(손으로 좌표를 옮기지 않는다. `capture-service` SPEC_PHONE **v5** 이상 — v4 이하의 meta는 탭 시각이 `idb ui tap`이 끝난 뒤라 링이 0.2초쯤 늦게 뜬다). 배열 `[{t,x,y}]`로 직접 줘도 된다(`t`는 **촬영본 시각** — `start`만큼 자동으로 당겨지고, 구간 밖은 안 그린다). `tapScale` 촬영본 픽셀 ÷ 포인트(기본 2) · `tapRadius` 끝 반지름(포인트, 기본 **24** = 지름이 화면 폭의 12%) · `tapDur` 길이(초, 기본 **0.34** — postTap보다 짧아 화면이 바뀌기 전에 끝난다) · `tapOpacity` 흰 띠 최대 알파(기본 **0.62**) · `tapWidth` 띠 두께(포인트, 기본 **2**). 좌표 변환은 눈이 아니라 계산이다 — 합성이 촬영본을 슬롯에 앉히는 그 식(cover 배율 + crop 오프셋)을 그대로 풀어 무대 좌표를 낸다 |
| `stand` | `false`면 받침대를 뺀다 · 숫자면 배율(1 = 원래 크기) |
| `mac` | `false`면 iMac·카피를 빼고 **배경만** 굽는다 — 빈 배경판 (`_motion/bg.json` → `bg.png`) |
