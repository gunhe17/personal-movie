...
<!-- 주체 — 동작의 시작점으로. "the moment before …".
     ⚠ 사람 명사를 쓰지 않는다 — "a woman's hand" · "to her ear" · 어깨 · 상반신 · 맞은편 사람은 얼굴을 부른다(첫 배치 8/26).
     손은 센다: "Exactly one hand and forearm in a grey sleeve enter from the right edge; the other hand is out of frame".
     글자가 있는 소품(달력·키패드·키캡·지침서·양식 라벨)은 프레임 밖으로 빼거나 무지 형태로 재정의한다 — 배제절은 본문 인쇄에만 먹는다. -->
<주체와 동작>. <표정·기색 한 마디>.
...
<비워둘 사분면 — 예: the entire right half is calm empty wall, softly defocused>, left clear for copy added later. The torso stays above the top edge of the frame.
