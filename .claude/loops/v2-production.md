...
- 2026-09-15 05:2x **폰 모양 화면끼리 표시 통일 — C1.9에서 마우스를 빼고 탭 링을 넣었다(t02).** C1.9만 시뮬레이터가 아니라 **웹을 폰 프로필로** 찍은 컷이라 데스크톱 화살표가 프레임에 구워져 있었다. 촬영 경로는 그대로 두고 표시만 바꿨다 — **SPEC v8→v9**: ① `--profile phone`이면 러너가 `--cursor off`(web은 그대로 `on` — 데스크톱 컷의 커서는 안 건드린다) ② `human.mjs`의 `click`이 `center()`로 이미 구한 좌표를 버리지 않고 `x`·`y`(CSS px)로 남기고, 시각을 `mousedown`~`up`의 **가운데**로 적는다(`phone.mjs` v5와 같은 뜻 — 눌린 뒤에 적으면 링이 이미 바뀐 화면 위에 편다). `target`은 안 바꿨다 — 기존 meta를 읽는 도구는 안 깨진다. `capture.md` 규칙 2·2-0-1 · capture-service/motion-stage SKILL.md 같이 고쳤다.
  - `render.mjs`의 `taps`가 `kind === 'click'`도 읽고 배율을 `capture.pxPerPoint ?? viewport.dpr`로 본다. **링 크기는 계산으로 맞췄다** — `tapRadius`가 논리 좌표(pt)라 dpr 3(390pt 폭)과 dpr 2(402pt 폭)에서 무대 지름이 **둘 다 96px**이다(C1.9 `scale 3 × f 0.6632` · C4.3 `scale 2 × f 1.0000`). 링을 굽는 `ringClip`이 배율 2를 하드코딩하고 있던 것이 진짜 버그였다 — meta에서 읽게 고쳤다. C4.3을 다시 구워 기존 완성본과 **PSNR inf**(비트 동일) 확인 · `--selftest` 통과.
  - **C1.9 t02**(`--retake-of t01`): 11.12s · notReady 0 · 충실도 100% · 시드 `_seed/saas-2026-09-15.json`. 완성본 **2.31–10.61 · 8.3s 그대로**(노컷의 뜻도 그대로 — 종이 위 칸을 채우는 구간). 탭 링 3(확인 · 정서·불안 · 또래관계). 프레임으로 확인: 마우스 없음 · 링이 **누르는 그 프레임**에서 체크 표시와 같이 뜬다.
- 2026-09-15 06:0x **S4 우리 스텝 2단계로 통합(사용자 지시).** `화자별 전사`·`일지 초안` 두 칸을 **`일지 작성`** 하나로 합쳤다 — C4.4·C4.5가 같은 2단계에 머문다(C5.5·C5.6과 같은 방식: 뒤 컷은 `head` 없이 `02.png` 유지).
  - S4-우리 3→**2**단계 재빌드, 옛 3단계 파일(02-03.mov · 03.png) 삭제. cuts.json steps + 무대 스펙 세 개(`C4.3.json`·`C4.4.json`·`C4.5.json`)의 steps 블록 같이 고침.
  - **영상 셋을 같은 파일명으로 덮어썼다** — 표시줄이 구워져 있어 새 라벨을 넣으려면 `raw/`에서 다시 굽는 수밖에 없다. `motion-stage/render.mjs <spec> --out <기존 파일명>`으로 제자리 재합성: `C4.3_phone.mp4`(13.40s) · `C4.4_imac.mp4`(4.00s) · `C4.5_imac.mp4`(7.30s). 길이·규격 그대로라 **Resolve 링크가 유지된다**(미디어 풀에서 새로고침만).
  - 스토리보드 재생성기를 `sb4.py`로 일반화 — 씬 이름을 인자로 받아 cuts.json + steps.json에서 섹션을 다시 짜고 손으로 쓴 `<p class="sub">`는 물려받는다.
  - 검사 추가: **cuts.json의 step 라벨 ↔ steps.json 배열**이 어긋나면 잡는다.
- 2026-09-15 06:2x **C3.6에 탭 링 — 폰 컷 다섯이 전부 같은 표시가 됐다(t09 · 9.3s 유지).** 앞선 두 세션이 "시드가 깨져 보류"로 남긴 컷이다. 다시 보니 **시드를 뒤집을 일이 없었다** — DB에 이미 9/18(금) 16:00 회기(`ca36e0de…036`)가 서 있고 t02가 만든 **대기 중인 변경 요청 한 건**만 붙어 있었다. 그 한 줄을 지우면 곧 C3.6의 출발 상태다. `c37-approve-setup.sql`(이수진을 `guardian.leesujin@`로 재링크)도, 계정 교체도 돌리지 않았다. 촬영이 만든 요청이 지운 것과 같은 요청(9/18 16:00 → 9/22 17:00 · 같은 person)이라 **DB는 작업 전 상태로 되돌아왔다.**
  - **탭 링 5개 · 무대 지름 96px** — C4.3·C1.9와 계산까지 같은 값이다. 프레임을 뽑아 눈으로 확인했다: 다섯 링이 전부 **누르는 그 화면 위**에 뜬다(일정 변경 요청 · 22일 · 17:00 · 변경 요청 · 이 시간으로 요청하기). 스와이프는 여전히 안 그린다.
  - **테이크를 다섯 태웠다(t04~t09).** 원인 둘. ① **콜드 런치 직후 첫 탭이 삼켜진다**(RN dev 빌드가 약 7초 뒤에야 반응한다) — 촬영 전에 앱을 종료→기동→홈에 세우고 한 번 눌러 데운다. ② **다른 작업의 ffmpeg이 load 22까지 올려 놓으면** 시뮬레이터가 굶고(전환 0.5초에 고유 프레임 4 · 정상 12~23) `idb ui describe-all`이 0.2초에서 4초로 늘어 라벨 조회가 실패한다. 시뮬레이터를 재부팅(4일 가동)하고 **부하가 가라앉은 창을 기다렸다가** 찍었다.
  - 채택 t09: 20.55s · notReady 0 · 충실도 100.1% · 고유 프레임 **156**(3~18.5s, 160×320) — 지금까지 쓰던 t03의 121보다 낫다. 완성본 **7.87–17.17 · 9.3s 그대로**, 노컷(변경 요청 → 확인까지 8.16–16.33)도 구간 안에 통째로 들어간다.
  - `until`을 넣은 `c36-request-v3.mjs`도 써 봤지만 **채택하지 않았다** — 이 앱의 변경 시트는 접근성 트리가 커서 `describe-all` 한 번이 1~4초다. `until`이 폴링할 때마다 그 값을 물어 조작이 15초에서 44초로 늘었다. **파일은 지웠다** — 쓰지 않는 스크립트를 남기지 않는다. 부하 높은 기계에서 라벨 조회가 계속 실패하면 전환 네 자리에 `p.until`을 도로 넣으면 된다(값은 위 실측대로 비싸다).
  - 원장: cuts.json C3.6 `source`·`segment` · briefs/S3.md C3.6 블록(옛 블록은 "6.0s"로 두 판 전이었다) · storyboard.html C3.6 썸네일(링이 있는 4.72초 프레임). **길이·컷 수는 안 바뀌었다** — C3.6은 9.3초 그대로라 합계도 그대로다(합계 자체는 옆 작업이 S4를 만지는 중이라 이 세션에서 다시 재지 않았다).
- 2026-09-15 06:4x **S7 정산 제거(사용자 지시).** C7.1·C7.2·C7.3·C7.4 넷 다 미편집이라 편집본에 영향 없다. `_dropped/`로 컷 폴더 다섯(`s07-c1`·`s07-c2`·`s07-c3-노쇼차감`·`s07-c3-두번째형광펜`·`s07-c4`) · 절차 표시줄 2트랙(`steps-S7-*`) · `briefs-S7.md`, 기록은 `s07-cuts-dropped.json`. **raw 폴더 `s07-정산/`은 그대로 둔다**(촬영 규칙 4). steps.json에서 S7 제거 → 남은 10트랙.
  - 씬 번호 고정 — S6·S7·S8 자리가 비고 **일곱 씬**(S0 S1 S2 S3 S4 S5 S9) **31컷 · 3:27.7**. 미편집은 셋만 남았다(C0.1 · C3.5 · C9.1).
  - 스토리보드 아티팩트 새로 냄: https://claude.ai/artifact/Q6CmTJ2PrhTgUCuksqi5g8 (옛 링크 872af4c1…는 이 세션에서 읽기가 막혀 갱신하지 못했다)
